...
class GitLogUnit(unittest.TestCase):
    def test_constructor__given_gov_uk__then_json_is_correct(self):
        ...
        subject = StackedHistogram(
            "new_date", "dir_1", "churn_count", filtered, "count"
        )

        filename = "temp_dir_1_govuk.png"
    ...
    def test_constructor__given_gov_uk__then_json_is_correct(self):
        ...
        print(filtered)
        subject = StackedHistogram(
            "new_date", "two_dirs", "churn_count", filtered, "count"
        )

        filename = "temp_two_dirs_govuk.png"
        subject.save_plot(filename)
